...
export function EveningReviewForm() {
  ...
  async function handleAnalyze() {
    ...
    try {
      const result = await generateNightlyReview({
        ...
        tasksCompleted,
        tasksTotal,
        // Use getTaskLabel so tasks with legacy "title" field also work correctly
        tasksSummary: todaysTasks.map((t) => ({
          content: getTaskLabel(t),
      ...
      toast({
        ...
        description: err?.message ?? 'Ocorreu um erro ao analisar o dia. Tente novamente.',
        variant: 'destructive',
      });
    } finally {
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
        ...
        >
          {isGenerating ? (
            ...
            <>
              ...
              {tasksTotal === 0 ? 'Sugerir tarefas com IA' : 'Analisar meu dia com IA'}
            </>
          )}
        </Button>
      )}
      ...
      {aiResult && suggestedTasks.length > 0 && (
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            {acceptedCount < suggestedTasks.length && (
              ...
              <CardContent className="pt-4 space-y-3">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 space-y-2">
                    <textarea
                      aria-label={`Conteúdo da sugestão ${index + 1}`}
                      value={task.editedContent}
                      onChange={(e) => updateContent(index, e.target.value)}
                      disabled={!task.accepted}
                      rows={2}
                      className="w-full text-sm bg-transparent resize-none border-0 focus:outline-none focus:ring-1 focus:ring-primary/50 rounded p-1 -ml-1"
                    />
      ...
      {/* ── SEÇÃO 6: Salvar ritual ── */}
      {aiResult && (
        <Button
          className="w-full"
          onClick={handleSave}
          disabled={isSaving || hasSaved || acceptedCount === 0}
        >
          {isSaving ? (
            <>
              <Loader2 className="mr-2 h-4 w-4 animate-spin" />
            ...
            <>
              <CheckCircle className="mr-2 h-4 w-4" />
              Salvar ritual e criar tarefas para amanhã ({acceptedCount})
            </>
          )}
        </Button>
      )}
